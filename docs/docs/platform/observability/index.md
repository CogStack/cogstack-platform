# Cogstack Observability

This project provides observability of a cogstack deployment.

It provides the following features:

- Dashboards for availability
- Telemetry of infrastructure such as host memory usage, and elasticsearch index size
- Alerting based on a defined Service Level Objective (SLO) and burn rates
- Blackbox Probing of services to find service level indicators of uptime and latency
- A working inventory of what is running where


See the [Quickstart](./get-started/quickstart.md) to see how to easily run this stack.

## Getting Started

- [Quickstart](./get-started/quickstart.md)
- [User Guide Tutorial](./get-started/userguide-tutorial.md)

## Setup

- [Production Setup](./setup/production-setup.md)
- [Probing](./setup/probing.md)
- [Telemetry](./setup/telemetry.md)
- [Alerting](./setup/alerting.md)

## Customization

- [Custom Dashboards](./customization/custom-dashboards.md)
- [Alerts Customization](./customization/alerts-customization.md)
- [Custom Prometheus Configs](./customization/custom-prometheus-configs.md)
- [Blackbox Exporter Config](./customization/blackbox-exporter-config.md)

## Reference

- [Project Details](./reference/project-details.md)
- [Concept Materials](./reference/concept-materials.md)
- [Quickstart Manual](./reference/quickstart-manual.md)
- [Understanding Metrics](./reference/understanding-metrics.md)

