  ...
  const deepClone = (value) => {
    ...
    if (typeof structuredClone === 'function') {
      try {
        ...
        // Reactive proxies cannot always be structured-cloned.
      }
    }
  ...
  return {
    ...
    async submit_query(temporal = null) {
      ...
      try {
        data = {
          qid,
          query: deepClone(this.query),
          filter: deepClone(this.filter),
          temporal: temporal == null ? null : deepClone(temporal),
        };
        requestBody = JSON.stringify(data);
      } catch (err) {
        console.log('submit_query payload clone error:', err);
        this.running_status = 'Something went wrong in the server, please try again later.';
        this.reset_results();
        this.clear_charts();
        this.is_busy = false;
      ...
      const isStale = () => this_qry_cnt !== this.qry_cnt || this.qid !== qid;
      console.log(data);
      ...
      if (isStale()) return;
      if (!query_result_resp.query_result || !query_result_resp.query_result.hasOwnProperty('all')) {
        this.running_status = 'Something went wrong in the server, please try again later.';
        this.reset_results();
        this.clear_charts();
        this.is_busy = false;
      ...
      if (this_qry_cnt == this.qry_cnt) {
        ...
        try {
          await this.draw_top_terms_chart(qid, data);
        } catch (err) {
    ...
    async draw_top_terms_chart(qid, querySnapshot = null) {
      //get data
      this.top_terms_chart_updating = true;
      let resp = {};
      const payload = querySnapshot || { qid, query: this.query, filter: this.filter };
      try {
      ...
      var top_terms_chart = echarts.init(document.getElementById('top_terms_chart'));
      var option = {
        series: [{type: 'treemap', roam: false, data: resp.data}],
        tooltip: [{show: true}],
      };
